  ...
  return (
    <div className="grid grid-cols-12 gap-6 h-[calc(100vh-100px)]">
      <div className="col-span-5 flex flex-col gap-4 h-full">
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-2 pb-10">
          {!loading && !error && filtered.map((e) => {
            ...
            return (
              <button key={e.id} onClick={() => setSelectedId(e.id)} className={`w-full text-left p-3 rounded-lg border transition-all ${isSelected ? 'bg-blue-50 border-blue-500' : 'bg-white border-slate-200'}`}>
                <div className="flex justify-between gap-3">
                  <div>
                    <div className="font-semibold text-slate-900 flex items-center gap-2">
                      <UserCircle2 size={18} /> {e.name}
                    </div>
                    <div className="text-xs text-slate-500 mt-0.5">{e.role} • {e.opStatus}</div>
                  </div>
      ...
      <div className="col-span-7 h-full overflow-hidden">
        {selected ? <EmployeeProfile employee={selected} initialTab={initialTab} /> : <div className="h-full flex items-center justify-center text-slate-400 border-2 border-dashed rounded-lg">Selecione um colaborador</div>}
      </div>
    </div>
  );
}
