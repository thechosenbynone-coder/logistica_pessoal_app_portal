...
export default function EmployeesPage() {
  ...
  return (
    <div className="grid grid-cols-12 gap-6 h-[calc(100vh-100px)] rounded-2xl bg-slate-950">
      <div className="col-span-5 flex flex-col gap-4 h-full">
        ...
        {loading && <div className="text-center text-sm text-slate-400">Carregando...</div>}
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-2 pb-10">
          {!loading && !error && filtered.map((e) => {
            ...
            return (
              <button key={e.id} onClick={() => setSelectedId(e.id)} className={`w-full text-left p-4 rounded-2xl border transition-all ${isSelected ? 'bg-blue-500/10 border-blue-500/60' : 'bg-slate-900/40 border-slate-700/50 hover:bg-slate-900/70'}`}>
                <div className="flex justify-between gap-3">
                  <div>
                    <div className="font-semibold text-slate-100 flex items-center gap-2">
                      <UserCircle2 size={18} /> {e.name}
                    </div>
                    <div className="text-xs font-mono text-slate-400 mt-0.5">{e.role} • {e.opStatus}</div>
                  </div>
      ...
      <div className="col-span-7 h-full overflow-hidden">
        {selected ? <EmployeeProfile employee={selected} initialTab={initialTab} /> : <div className="h-full flex items-center justify-center text-slate-500 border-2 border-slate-700/70 border-dashed rounded-2xl bg-slate-900/30">Selecione um colaborador</div>}
      </div>
    </div>
  );
}
