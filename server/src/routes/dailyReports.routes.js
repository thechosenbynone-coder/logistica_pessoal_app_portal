...
router.patch('/api/daily-reports/:id/review', async (req, res) => {
  try {
    ...
    handleServerError(res, error, 'daily-reports-review');
  }
});
...
router.get('/api/rdo/sem-preenchimento', async (req, res) => {
  try {
    ...
    handleServerError(res, error, 'rdo-sem-preenchimento');
  }
});
...
router.get('/api/employees/:id/daily-reports', ...employeeParamsAuth, async (req, res) => {
  try {
    ...
    res.json(rows.map(mapDailyReport));
  } catch (error) {
  ...
  try {
    ...
    if (emp && emp.currentStatus !== 'EMBARCADO') {
      return res.status(403).json({
        ...
        message: `RDO só pode ser criado quando o colaborador está embarcado. Status atual: ${emp.currentStatus || 'BASE'}`,
      });
    }

    const reportDateParsed = parseDateInputOrError(data.report_date, 'report_date');
    if (reportDateParsed.error)
      return res.status(400).json({ errorCode: 'VALIDATION_ERROR', message: reportDateParsed.error });
